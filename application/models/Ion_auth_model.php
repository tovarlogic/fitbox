...
class Ion_auth_model extends CI_Model
{
	...
	{
		if ((empty($id) AND $id != 0) OR empty($password))
		{
			log_message('DEBUG',print_r('error1:'.$password,TRUE));
			return FALSE;
		}
		...
		if ($query->num_rows() !== 1)
		{
			log_message('DEBUG',print_r('error2:'.$password,TRUE));
			return FALSE;
		}
	...
	public function salt()
	{
		...
		if (!$buffer_valid OR strlen($buffer) < $raw_salt_len)
		{
			$bl = strlen($buffer);
	...
	///////////////// CUSTOM KINSAY
	public function set_password($identity, $new)
	{
		$this->trigger_events('pre_change_password');
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('identity_cookie_name', 'ion_auth'))
			OR !get_cookie($this->config->item('remember_cookie_name', 'ion_auth'))
			OR !$this->identity_check(get_cookie($this->config->item('identity_cookie_name', 'ion_auth'))))
		{
