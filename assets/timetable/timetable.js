...
//			scroll to active timetable
	...
	function calc_popup_params()
	{
		...
		var window_height,
			window_width,
			offset_top;
		if(in_iframe() && navigator.userAgent.match(/(iPod|iPhone|iPad)/))
		{
			window_height = $(parent.window).height();
			window_width = $(parent.window).width();
			offset_top = ($(document).scrollTop()>=$(parent.document).scrollTop() ? $(document).scrollTop() : $(parent.document).scrollTop());
		}
		else
		{
