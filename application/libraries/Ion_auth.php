...
///
	...
	public function check_login($groups, $redirect = true)
    {
        if (!$this->logged_in() )
        {
            $this->session->set_userdata('referred_from', current_url());

            if ($this->input->post('ajax') OR $this->input->is_ajax_request() OR $redirect == false)
                return FALSE;
            else
    ...
    public function redirect_by_group()
	{
		...
		if ( $this->ion_auth->in_group('athlete') AND $this->ion_auth->in_group($staff) )
			redirect(base_url() . 'select', 'refresh');

		else if ($this->ion_auth->in_group('athlete'))
            redirect(base_url() . 'athlete', 'refresh');

        else if ($this->ion_auth->in_group('guest'))
            redirect(base_url() . 'guest', 'refresh');

        else if ($this->ion_auth->in_group('board'))
            redirect(base_url() . 'board', 'refresh');

        else if ($this->ion_auth->in_group($staff))
            redirect(base_url() . 'staff', 'refresh');

        else if ($this->ion_auth->in_group('sudo'))
            redirect(base_url() . 'sudo', 'refresh');       
	}
	...
	function redirectReferer()
	{
		...
		{
			$this->ion_auth->redirect_by_group();
		}
	}
	...
	public function send_password($password, $email, $name)
	{
		$data = array(
				'password'   => $password,
			);
		$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_password', 'ion_auth'), $data, true);

		$this->email->clear();
		$this->email->from($this->config->item('admin_email', 'ion_auth'), $this->config->item('site_title', 'ion_auth'));
		$this->email->to($email);
		$this->email->subject($this->config->item('site_title', 'ion_auth') . ' -  contraseña temporal');
		$this->email->message($message);
